...
async function createPolyfillsData(cfg) {
    ...
    if (polyfills.systemjs ||
        ...
        (!hasSystemJs && (0, utils_1.hasFileOfType)(cfg, utils_1.fileTypes.SYSTEMJS))) {
        const name = 'systemjs';
    ...
    for (const polyfillConfig of polyfillConfigs) {
        ...
        const filePath = `${path_1.default.posix.join(cfg.polyfillsDir || 'polyfills', `${polyfillConfig.name}${polyfills.hash !== false ? `.${(0, utils_1.createContentHash)(content)}` : ''}`)}.js`;
        const polyfillFile = {
